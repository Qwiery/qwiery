...
module.exports = function (app, qwiery) {
    ...
    const oracle = qwiery.services.oracle;
    const history = qwiery.services.history;
    const identity = qwiery.services.identity;
    const personalization = qwiery.services.personalization;
    ...
    app.use('/lexic/randomQuestion', security.ensureApiKey, async function (req, res) {
        const r = await oracle.random();
        const n = await oracle.randomNouns(3);
        const nouns = n.split(',');
        let selected = r.Questions;
        if (_.isArray(r.Questions)) {
     ...
     */
    app.post('/authentication/connectFacebook', async function (req, res) {
