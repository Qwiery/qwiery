...
class WorkflowEngine extends ServiceBase {
    ...
    async run(def, inputs) {
        ...
        const spy = await that.runWorkflow(def, session);
        flowId = spy.flow.id;
        ...
        async function next() {
            if (inputs.length > 0) {
                ...
                const activeFlow = await that.getWorkflowById(flowId, session.Context);
                ...
                if (utils.isUndefined(activeFlow)) {
                    ...
                    await that.continueWithWorkflow(activeFlow, session, spy);
