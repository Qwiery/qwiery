...
class WorkflowEngine extends ServiceBase {
     ...
     * @param session
     */
    resolveInterruptedWorkflow(workflow, session) {
        ...
        function runner() {
            ...
            if(utils.isUndefined(boolish)) {
                ...
                waitFor(that.workflowStorage.upsertWorkflow(workflow, session.Context));
                session.Output.Answer = [{
                    DataType: "Text",
                ...
                waitFor(that.workflowStorage.deleteWorkflow(workflow, session.Context));
                session.Output.Answer = [{
                    DataType: "Text",
     ...
     * @returns {Promise<T>}
     */
    runWorkflow(workflow, session, spy) {
        ...
        function runner() {
            if(_.isPlainObject(workflow)) {
                ...
                let def = waitFor(that.getLibraryItem(workflow));
            ...
            return waitFor(instance.start(session, spy));
        }
     ...
     */
    resumeWorkflow(workflowDefinition, session, spy) {
        ...
        function runner() {
            ...
            let spion = waitFor(wf.start(session, spy));
            return spion;
        }
