...
class WorkflowState {
    ...
    deactivate() {
        ...
        function runner() {
            if(that.onDeactivate) {
                ...
                if(that.workflow) {
                    const message = waitFor(that.workflow.interprete(that.deactivateMessage));
     ...
     * @memberOf WorkflowState
     */
    accept(transitionValue = true) {
        const that = this;
        function runner() {
            if(that.onAccept) {
                if(that.workflow) {
                    const message = waitFor(that.workflow.interprete(that.acceptMessage));
            ...
            if(that.workflow) {
                waitFor(that.workflow._acceptedExecution(transitionValue, that));
            }
        }
    ...
    reject(reason) {
        ...
        function runner() {
            if(that.onReject) {
                if(that.workflow) {
                    const message = waitFor(that.workflow.interprete(that.rejectMessage));
            ...
            if(that.workflow) {
                waitFor(that.workflow._rejectedExecution());
            }
        }
