...
class Workflow {
    ...
    constructor(definition) {
        this.id = definition.Id || utils.randomId();
        /**
    ...
    start(session, spy) {
        ...
        return new Promise(function(resolve, reject) {
            ...
            loader.load(that.definition, that)
                .then(function() {
                    if(that._isActive) {
                        that._trigger(session, spy).then(function() {
                            resolve(spy);
                        });
                    } else {
                        that._enter(session, spy).then(function() {
                            resolve(spy);
                        });
                    }
                });
    ...
    _enter(session, spy) {
        ...
        function runner() {
            ...
            if(utils.isUndefined(that.currentStateName) || that.currentStateName.length === 0) {
                waitFor(that._moveToInitialState());
            }

            // a single-state should never be saved
            if(that.currentState.isInitial && that.currentState.isFinal) {
                that.neverSave = true;
                waitFor(that.currentState.activate());
                waitFor(that._checkAndHandleFinalState());
            } else {
                const dosave = waitFor(that.currentState.activate());
                if(dosave) {
                    waitFor(that._upsertWorkflow());
                }
            }
            return spy;
        }
    ...
    _trigger(session, spy) {
        ...
        function runner() {
            ...
            const doesQuit = waitFor(that._isQuitting());
            if(!doesQuit) {
                waitFor(that._executeCurrentState());
            }
            return spy;
        }
    ...
    interprete(obj) {
        ...
        // if(_.isString(obj)) {
        //     return Workflow._replaceVariables(obj, workflow);
        // }
        // const result = utils.makeChoices(obj);
        // if(_.isArray(result)) {
        //     let summ = "";
        ...
        //                 case "summary":
        //                     summ += workflow._summarizeWorkflow();
        //                     break;
        //             }
        ...
        //     return Workflow._replaceVariables(result, workflow);
        // }
        // else if(_.isObject(result)) { // most likely an entity
        //     const that = this;
        //     _.forOwn(result, function(v, k) {
        //         result[k] = Workflow._replaceVariables(result[k], workflow);
        //     });
        //     return result;
        // }
        // return result;
     ...
     * @private
     */
    _checkAndHandleFinalState() {
        const that = this;
        if(this.currentState.isFinal === true) {
            return new Promise(function(resolve, reject) {
                that._executeCurrentState().then(function() {
                    that.currentState.deactivate().then(function() {
    ...
    _moveToState(newStateName) {
        ...
        return new Promise(function(resolve, reject) {
            that.currentState.deactivate().then(function() {
                ...
                that.currentState.activate().then(function(dosave) {
                    resolve(dosave);
                });
            });
        });
