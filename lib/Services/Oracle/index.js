...
class Oracle extends ServiceBase {
     ...
     */
    async askUntilDone(question, stack, session, userSpecific) {
        const that = this;

        const oracleResponse = await (that.askOnce(question, session, userSpecific));
        if (oracleResponse !== null) {
            ...
            if (Oracle.isTemplateRedirect(oracleResponse.Template)) {
                ...
                stack = await (that.askUntilDone(newQuestion, stack, session, userSpecific));
            }
        }
        return stack;
    }
     ...
     * @returns {*}
     */
    async askOnce(question, session, userSpecific) {
        ...
        if (utils.isDefined(appOracleItems) && appOracleItems.length > 0) {
            ...
            if (userSpecific === true) {
                subset = await (that.oracle.getSubset(question, session.BotConfiguration.categories, session.Key.UserId));
            } else {
                subset = await (that.oracle.getSubset(question, session.BotConfiguration.categories));
            }
        }
        ...
        if (_.isNil(found.bestOption.Template) || utils.isUndefined(found.bestOption.Template.Answer)) {
    ...
    async learn(qtl) {
        ...
        await (that.validate(qtl));
        const found = utils.isDefined(qtl.Id) ? await (that.findId(qtl.Id)) : null;
        qtl = {
            Id: qtl.Id || -1,
        ...
        if (utils.isDefined(found)) {
            ...
            if (previousCategory !== qtl.Category) {
                await (that.oracle.changeCategory(found, previousCategory, qtl.Category));
            }
            // update data
