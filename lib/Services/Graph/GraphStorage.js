...
class GraphStorage extends StorageDomainBase {
    ...
    async getWorkspaceFromNodeId(id, userId) {
        const that = this;

        let udata = await (that.getUser(userId));
        const nodes = await (that.getAccessibleNodes(userId));
        ...
        if (utils.isDefined(found)) {
            ...
            if (found.UserId !== userId) { // happens if the node sits in a shared space
                udata = await (that.getUser(found.UserId));
            }
     ...
     */
    async setCurrentWorkspace(name, userId) {
        const that = this;

        const udata = await (that.getUser(userId));
        name = name.trim();

        const found = await (that.getWorkspaceIdByName(name, userId));
        if (utils.isDefined(found)) {
            udata.Metadata.CurrentWorkspace = found;
            await (that.save(udata));
            return true;
        } else {
            return false;
        }
    ...
    async deleteWorkspace(workspaceId, userId) {
        ...
        for (let i = 0; i < space.Users.length; i++) {
            ...
            await (that.revokeAccessToWorkspace(workspaceId, otherUserId, userId));
        }
    ...
    async createGraph(graph, userId) {
        const that = this;
        ...
        for (let k = 0; k < entities.length; k++) {
            ...
            await (that.upsertEntity(node, userId));
        ...
        for (let k = 0; k < links.length; k++) {
            ...
            link.UserId = userId;
            if (!await (that.areConnected(link.IdSource, link.IdTarget, userId))) {
                await (that.connect(link.IdSource, link.IdTarget, link.Title || null, userId))
    ...
    async getRelated(id1, userId) {
        ...
        if (ids.length > 0) {
            for (let i = 0; i < ids.length; i++) {
                const item = ids[i];
                const node = await (that.getAccessibleNode(item.id, userId));
    ...
    async etLinksBetween(id1, id2, userId) {
        const that = this;
        const udata = await (that.getUser(userId));
    ...
    async getTriplesByTitle(startTitle, endTitle, userId) {
        const that = this;
        ...
        if (utils.isDefined(endTitle)) {
            ...
            if (utils.isDefined(to)) {
                const links = await (this.getLinksBetween(from.Id, to.Id, userId));
                const predicates = [];
            ...
            const related = await (that.getRelated(from.Id, userId));
    ...
    async getRecent(userId) {
        ...
        for (let i = 0; i < recentIds.length; i++) {
            ...
            const node = await (that.getNode(id, userId));
            result.push(node);
        }
        return result;
    }
    ...
    async getAccessibleNodes(userId) {
        ...
        if (udata.Metadata.AccessTo) {
            for (let i = 0; i < udata.Metadata.AccessTo.length; i++) {
                ...
                const odata = await (that.getUser(access.UserId));
     ...
     * already exists.
     */
    async createSemanticGraph(graph, userId) {
        const that = this;
        const udata = await (that.getUser(userId));
        const entities = graph.Nodes;
        const links = graph.Links;
        ...
        for (let k = 0; k < entities.length; k++) {
            const node = entities[k];
            node.UserId = userId; // owner
            const newOrExistingId = await (that.createThought(node.Title, userId));
     ...
     */
    async upsertEntity(node, userId) {
        const that = this;
        const udata = await (that.getUser(userId));
        if (utils.isUndefined(udata)) {
            return null;
        }
        const workspaceId = await (that.getCurrentWorkspaceId(userId));

        const existingNode = await (that.getNode(node.Id, userId));
        ...
        if (isNew) {
            ...
            const space = await (that.getWorkspace(workspaceId, userId));
            if (space.IsPublic) {
    ...
    async upsertEntities(entities, userId) {
        const that = this;
        const r = [];
        for (let i = 0; i < entities.length; i++) {
            const entity = entities[i];
            const node = await (that.upsertEntity(entity, userId));
            r.push(node.Id);
        }
        return r;
    }
     ...
     */
    async attachTag(tagName, id, userId) {
        const that = this;
        const udata = await (that.getUser(userId));
        if (utils.isUndefined(udata)) {
            return null;
     ...
     * @return {Promise<TResult>}
     */
    async detachTag(tagName, id, userId) {
        const that = this;
        const udata = await (that.getUser(userId));
        if (utils.isUndefined(udata)) {
            return null;
        ...
        const ents = await (that.getTagEntities(tagName, userId));
    ...
    async getThoughts(userId) {
        const that = this;
        const result = [];
        const entities = await (that.getAllUserOwnedEntities(userId));
        for (let i = 0; i < entities.length; i++) {
            const entity = entities[i];
            if (entity.DataType === 'Thought') {
                const image = await (that.getEntityRandomImage(entity.Id, userId));
    ...
    async getTagEntities(tagName, userId) {
        const that = this;
    ...
    async makeWorkspacePublic(workspaceId, userId) {
        const that = this;
    ...
    async makeWorkspacePrivate(workspaceId, userId) {
        const that = this;
        const udata = await (that.getUser(userId));
        if (utils.isUndefined(udata)) {
            return null;
    ...
    async grantAccessToWorkspace(workspaceId, otherUserId, userId) {
        const that = this;

        const udata = await (that.getUser(userId));
        const odata = await (that.getUser(otherUserId));
        if (utils.isUndefined(udata) || utils.isUndefined(odata)) {
            return null;
        }
        if (!await (that.userExists(userId))) {
        ...
        await (that.save(udata));
        await (that.save(odata));

    }

    async revokeAccessToWorkspace(workspaceId, otherUserId, userId) {
        const that = this;
    ...
    async getUserDefaultWorkspace(otherUserId, userId) {
        const spaces = await (this.getWorkspacesOfUser(otherUserId, userId));
        return _.find(spaces, {'IsDefault': true});
    }

    async getLinksBetween(id1, id2, userId) {
        const udata = await (this.getUser(userId));
        const links = udata.Links;
        const coll = [];
        if (_.isNil(links) || links.length === 0) {
            return coll;
        }
        for (let i = 0; i < links.length; i++) {
            const link = links[i];
            if ((link.IdSource === id1 && link.IdTarget === id2) || (link.IdSource === id2 && link.IdTarget === id1)) {
                coll.push(link);
            }
        }
        return coll;
