...
class GraphStorage extends StorageDomainBase {
    ...
    connect(id1, id2, label, userId) {
        ...
        function runner() {
            ...
            const targetSpace = waitFor(that.getWorkspaceFromNodeId(id2, userId));
            ...
            const found = waitFor(that.findDirectedLink(id1, id2, label, userId));
     ...
     */
    search(term, type, userId, count = 1000) {
        const that = this;

        function runner() {
            term = term.trim().replace(/\*/gi, "(.*?)");
            const found = [];
            const ents = waitFor(that.getAccessibleNodes(userId));
