...
class Apps extends ServiceBase {
    ...
    deleteApp(appId, ctx) {
        ...
        function runner() {
            ...
            waitFor(that.storage.Oracle.deleteCategory(appId));

            // delete the configuration of the app
            waitFor(that._deleteAppConfiguration(appId));

            // finally, the user reference
            waitFor(that.userApps.removeUserApp(ctx, appId));
    ...
    _validateAppConfig(appConfig) {
        ...
        function runner() {
            ...
            else if (waitFor(that.isExistingAppName(appConfig.config.name))) {
