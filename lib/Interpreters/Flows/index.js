...
class Flows extends InterpreterBase {
    ...
    processMessage(session) {
        ...
        function runner() {
            ...
            const interruptedWorkflow = waitFor(that.services.workflows.getInterruptedWorkflow(ctx));
            let resolution;
            if(utils.isDefined(interruptedWorkflow)) {
                ...
                const activeWorkflow = waitFor(that.services.workflows.getActiveWorkflow(ctx));

                if(utils.isDefined(activeWorkflow)) {
                    // so, let's continue with that workflow, shall we?
                    let spy = waitFor(that.services.workflows.continueWithWorkflow(activeWorkflow, session));
