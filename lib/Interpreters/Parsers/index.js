...
class Parsers extends InterpreterBase {
    ...
    keywords(session) {
        try {
            session.Keywords = waitFor(Language.getKeywords(session.Input.Raw, 1, true)).split(",");
        } catch(e) {
            session.Keywords = [];
        }
        return session;
    ...
    language(session) {
        ...
        try {
            if(utils.isDefined(firstCommand)) {
                ...
                let found = waitFor(Language.detectLanguage(session.Input.Raw));
    ...
    sentiment(session) {
        try {
            session.Sentiment = waitFor(Language.detectSentiment(session.Input.Raw));
        } catch(e) {
            session.Sentiment = 0;
        }
        return session;
