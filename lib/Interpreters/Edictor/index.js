...
class Edictor extends InterpreterBase {
     ...
     * @private
     */
    async _processMessage(session, resolve, reject) {

        const question = session.Input.Raw;
        const that = this;

        // first test whether there is a user-specific answer
        let stack = await (that.oracle.ask(question, session, true));
        if (stack === null || stack.length === 0) {
            // use the global oracle to try to answer the question
            stack = await (that.services.oracle.ask(question, session, false));
        }
        if (stack === null || stack.length === 0) {
     ...
     * @private
     */
    async _processTemplate(session, template) {
        ...
        const mutated = await (that.qtl.mutateOracleItem(template, session));

        const executed = await (that.executeTemplate(mutated, session, ctx));
        ...
        // fetches the node
        if (answer.GetNode) {
            const node = await (this.graph.getAccessibleNode(answer.GetNode, ctx));
     ...
     * @returns {*}
     */
    async executeTemplate(oracleItem, session, ctx) {
        const that = this;
        ...
        if (oracleItem.Topics && oracleItem.Topics.length > 0) {
            ...
            if (that.topics) {
                await (that.topics.addUserTopics(oracleItem.Topics, ctx));
            }
        }
        if (oracleItem.Template.Think) {
            ...
            if (think.Create) {
                ...
                if (think.Create.Graph.Semantic === true) {
                    await (that.graph.createSemanticGraph(think.Create.Graph, ctx));
                } else {
                    // when tasks and such are created in QTL
                    await (that.graph.createGraph(think.Create.Graph, ctx));
                }
            }
            // change parameters
            if (think.Context) {
                await (that.executeContext(think.Context, ctx));
            }
            ...
            if (think.CreateReturn) {
                ...
                if (think.CreateReturn.Workflow) {
                    try {
                        let spy = await (that.executeWorkflow(think.CreateReturn.Workflow, session));
                    ...
                    if (think.CreateReturn.Graph.Semantic === true) {
                        newGraph = await (this.graph.createSemanticGraph(think.CreateReturn.Graph, ctx));
                    } else {
                        // when tasks and such are created in QTL
                        newGraph = await (that.graph.createGraph(think.CreateReturn.Graph, ctx));
                    }
