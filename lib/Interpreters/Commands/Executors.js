...
class Executors {
    ...
    static deleteTag(instruction, session, command) {
        const that = this;
        ...
        function runner() {
            if(instruction.Commands.length > 2) {
                ...
                if(instruction.HasNamedArguments) { // delete>tag> name: favorites, id: EA44fqsf
                    ...
                    let spy = waitFor(command.services.workflows.runWorkflow(deletionWorkflow, session));
                    return spy.toAnswer();
                }
            }
        }
    ...
    static deleteEntity(instruction, session, command) {
        const that = this;
    ...
    static addTag(instruction, session, command) {
        function runner() {
            if(instruction.Commands.length > 2) {
                ...
                waitFor(command.services.graph.addTag(instruction.FirstParameter.value, session.Context));
    ...
    static addTask(instruction, session, command) {
        function runner() {
            if(instruction.Commands.length > 2) {
                ...
                waitFor(command.services.graph.upsertEntity(d, session.Context));
    ...
    static addPerson(instruction, session, command) {
        function runner() {
            if(instruction.Commands.length > 2) {
    ...
    static addAddress(instruction, session, command) {
        function runner() {
            if(instruction.Commands.length > 2) {
    ...
    static addPersonalization(instruction, session, command) {
        function runner() {
            ...
            if(instruction.Commands.length > 2) {
                ...
                waitFor(command.services.personalization.addPersonalization(name, value, session.Context));
    ...
    static addAppointment(instruction, session, command) {

        function runner() {
            if(instruction instanceof Entities.Appointment) {
                waitFor(command.services.graph.upsertEntity(instruction.toJSON(), session.Context));
                ...
                if(instruction.Commands.length > 2) {
                    ...
                    waitFor(command.services.graph.upsertEntity(ag.toJSON(), session.Context));
                    return Executors.messagePods("The appointment has been added.");
                }
            }
        }
    ...
    static addSpace(instruction, session, command) {
        function runner() {
            if(instruction.Commands.length > 2) {
                return Executors.messagePods(Executors.NotOK + "If you want to add a workspace, try with e.g. 'add>space> my new space'.");
            } else {
                const exists = waitFor(command.services.graph.getWorkspaceIdByName(instruction.FirstParameter.value, session.Context));

                if(exists) {
                    return Executors.messagePods(`The space '${instruction.FirstParameter.value}' exists already.`);
                } else {
                    const workspaceId = waitFor(command.services.graph.addWorkspace({Name: instruction.FirstParameter.value}, session.Context));
    ...
    static deleteSpace(instruction, session, command) {
        const that = this;
    ...
    static setSpace(instruction, session, command) {
        const that = this;
        if(instruction.Commands.length > 2) {
            ...
            let changed = waitFor(command.services.graph.setActiveWorkspace(instruction.FirstParameter.value, session.Context));
            if(changed) {
    ...
    static getHelp(instruction, session, command) {
            ...
            if(instruction.Commands.length > 1) {
                ...
                if(command.services.documents) {
                    if(utils.isUndefined(instruction.FirstParameter)) {
                        ...
                        const content = waitFor(command.services.documents.getDocTopic(instruction.FirstParameter.value));
