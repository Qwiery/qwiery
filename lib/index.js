...
class Qwiery {
     ...
     * @private
     */
    async _createNewSession(question, settings = {}) {
        const that = this;
        ...
        if (that.services.apps) {
            configuration = await (that.services.apps.getAppConfiguration(settings.appId));
        }
    ...
    async ask(question, options = {}) {
        ...
        if (that.settings.system.checkIdentity === true) {
            ...
            let exists = await (that.services.identity.exists(settings.userId));
            if (!exists) {
        ...
        let info;
        if (utils.isUndefined(question)) {
            let session = await (that._createNewSession('', settings));
        ...
        if (cleanInput.length === 0) {
            let session = await (that._createNewSession(cleanInput, settings));
            session.Output.Answer = utils.messagePods(Language.getVariation(constants.EMPTYINPUT));
        ...
        if (utils.isUndefined(settings.appId)) {
            ...
            if (entities.mentions.length > 0) {
                ...
                if (foundAppNames.length === 0) {
                    ...
                    settings.appId = await (that.services.apps.getAppIdFromName(appName));
                    if (utils.isUndefined(settings.appId)) {
        ...
        if (that.services.pipeline) {
            session = await (that.services.pipeline.processMessage(session));
        } else {
     ...
     * @param question {string} anything you like to ask
    ...
    /**
     * An alias for AskFlat.
     */
    askSimple(question, ctx, req) {
        return askFlat(question, ctx, req);
    }
